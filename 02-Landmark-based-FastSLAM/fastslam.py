    ...
    def correction_step(self, sensor_measurements):
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                ...
                angle = robot_pose[2] + measurement.z_bearing
                normalized_angle = normalize_angle(angle) 
                landmark.mu = robot_pose[0:2] +  [[measurement.z_range * cos(normalized_angle)], [measurement.z_range * sin(normalized_angle)]]
                ...
                landmark.sigma = np.dot(np.dot(inverse_H, Q_t), inverse_H.T)
                ...
                Q = np.dot(np.dot(H, landmark.sigma), H.T) + Q_t

                # TODO: calculate the Kalman gain
                K = np.dot(np.dot(landmark.sigma, H.T), np.linalg.inv(Q))
                ...
                landmark.mu = landmark.mu + np.dot(K, error)
                landmark.sigma = np.dot((np.identity(2) - np.dot(K, H)), landmark.sigma)
                
                # TODO: compute the likelihood of this observation, multiply with the former weight
                # to account for observing several features in one time step
                self.weight *= 1 / np.sqrt(np.linalg.det(2 * np.pi * Q)) * np.exp(-0.5 * np.dot(np.dot(error.T, np.linalg.inv(Q)), error))
...
def main():
    ...
    landmarks_truth = read_world('output/data/world.dat')

    # Read sensor readings, i.e. odometry and range-bearing sensor
    data = read_sensor_data('output/data/sensor_data.dat')
